...
export default class CalHeatMap {
    ...
    DEFAULT_LEGEND_MARGIN: number;
    NAVIGATE_LEFT: number;
    NAVIGATE_RIGHT: number;
    ...
    root: any;
    tooltip: any;
    verticalDomainLabel: any;
    private _completed: any;
    private _domains: any;
    private _domainType: any;
    private _init: any;
    private _maxDomainReached: any;
    private _minDomainReached: any;
    constructor() {
        "use strict";
        ...
        for (let type in this._domainType) {
            if (this._domainType.hasOwnProperty(type)) {
                let d = this._domainType[type];
        ...
        this._init = function() {
            ...
            self.tooltip = select(self.options.itemSelector)
                .attr("style", function() {
                    let current = select(self.options.itemSelector).attr("style");
        ...
        function _initCalendar() {
            ...
            let domains = self.getDomainKeys();
        ...
        function w(d, outer) {
            let width = self.options.cellSize * self._domainType[self.options.subDomain].column(d) + self.options.cellPadding * self._domainType[self.options.subDomain].column(d);
        ...
        function h(d, outer) {
            let height = self.options.cellSize * self._domainType[self.options.subDomain].row(d) + self.options.cellPadding * self._domainType[self.options.subDomain].row(d);
            if (arguments.length === 2 && outer === true) {
        ...
        this.paint = function(navigationDir) {
            ...
            let domainSvg = self.root.select(".graph")
                .selectAll(".graph-domain")
                .data(
                    function() {
                        let data = self.getDomainKeys();
            ...
            // PAINTING SUBDOMAINS														//
            // =========================================================================//
            let subDomainSvgGroup = svg.append("svg")
                .attr("x", function() {
                ...
                .call(function(selection) {
                    ...
                    if (options.tooltip) {
                        selection.on("mouseover", function(d) {
                            let domainNode = this.parentNode.parentNode;

                            self.tooltip
                                .html(self.getSubDomainTitle(d))
                                .attr("style", "display: block;");

                            let tooltipPositionX = self.positionSubDomainX(d.t) - self.tooltip.node().offsetWidth / 2 + options.cellSize / 2;
                            let tooltipPositionY = self.positionSubDomainY(d.t) - self.tooltip.node().offsetHeight - options.cellSize / 2;
            ...
            let tempWidth = self.graphDim.width;
            let tempHeight = self.graphDim.height;
    ...
    init(settings) {
        ...
        let options = parent.options = mergeRecursive(parent.options, settings);
        ...
        let s = ["data", "onComplete", "onClick", "afterLoad", "afterLoadData", "afterLoadPreviousDomain", "afterLoadNextDomain"];
    ...
    fill(svg) {
        ...
        rect.transition().duration(options.animationDuration).select("rect")
            .attr("class", function(d) {
                ...
                let htmlClass = parent.getHighlightClassName(d.t).trim().split(" ");
                let pastDate = parent.dateIsLessThan(d.t, new Date());
                let sameDate = parent.dateIsEqual(d.t, new Date());
    ...
    onComplete() {
        ...
        let response = this.triggerEvent("onComplete", [], this._completed);
    ...
    afterLoadPreviousDomain(start) {
        ...
        return this.triggerEvent("afterLoadPreviousDomain", function() {
            let subDomain = parent.getSubDomain(start);
    ...
    loadNextDomain(n) {
        ...
        let bound = this.loadNewDomains(this.NAVIGATE_RIGHT, this.getDomain(this.getNextDomain(), n));
    ...
    loadPreviousDomain(n) {
        ...
        let bound = this.loadNewDomains(this.NAVIGATE_LEFT, this.getDomain(this.getDomainKeys()[0], -n).reverse());
    ...
    loadNewDomains(direction, newDomains) {
        ...
        let parent = this;
        let backward = direction === this.NAVIGATE_LEFT;
        let i = -1;
    ...
    positionSubDomainX(d) {
        ...
        let index = this._domainType[this.options.subDomain].position.x(new Date(d));
    ...
    positionSubDomainY(d) {
        ...
        let index = this._domainType[this.options.subDomain].position.y(new Date(d));
        return index * this.options.cellSize + index * this.options.cellPadding;
    }
    ...
    getHighlightClassName(d) {
        ...
        if (this.options.highlight.length > 0) {
            for (let i in this.options.highlight) {
    ...
    getWeekNumber(d) {
        ...
        let f = this.options.weekStartOnMonday === true ? timeFormat("%W") : timeFormat("%U");
        return f(d);
    }
    ...
    getMonthWeekNumber(d) {
        ...
        let monthFirstWeekNumber = this.getWeekNumber(new Date(d.getFullYear(), d.getMonth()));
    ...
    getHourDomain(d, range) {
        ...
        let domains = timeHours(Math.min(start, stop), Math.max(start, stop));
    ...
    getMonthDomain(d, range) {
        ...
        let start = new Date(d.getFullYear(), d.getMonth());
        let stop = null;
        if (range instanceof Date) {
    ...
    getYearDomain(d, range) {
        ...
        let start = new Date(d.getFullYear(), 0);
        let stop = null;
        if (range instanceof Date) {
    ...
    getDomain(date, range) {
        ...
        switch (this.options.domain) {
            case "hour":
                let domains = this.getHourDomain(date, range);
    ...
    getSubDomain(date) {
         ...
         * @return int
         */
        let computeDaySubDomainSize = function(date, domain) {
            switch (domain) {
                case "year":
         ...
         * @return int
         */
        let computeMinSubDomainSize = function(date, domain) {
            switch (domain) {
                case "hour":
                    return 60;
                case "day":
         ...
         * @return int
         */
        let computeHourSubDomainSize = function(date, domain) {
            switch (domain) {
                case "day":
                    return 24;
                case "week":
         ...
         * @return int
         */
        let computeWeekSubDomainSize = function(date, domain) {
            if (domain === "month") {
                let endOfMonth = new Date(date.getFullYear(), date.getMonth() + 1, 0);
                let endWeekNb = parent.getWeekNumber(endOfMonth);
                let startWeekNb = parent.getWeekNumber(new Date(date.getFullYear(), date.getMonth()));
        ...
        switch (typeof source) {
            case "string":
                if (source === "") {
                    ...
                    let url = this.parseURI(source, startDate, endDate);
                    let requestType = "GET";
    ...
    parseDatas(data, updateMode, startDate, endDate) {
        ...
        for (let d in data) {
            ...
            let subDomainsData = this._domains.get(domainUnit);

            if (!temp.hasOwnProperty(domainUnit)) {
                temp[domainUnit] = subDomainsData.map(extractTime);
            }

            let index = temp[domainUnit].indexOf(this._domainType[this.options.subDomain].extractUnit(date));
    ...
    resize() {
        ...
        let parent = this;
        let options = parent.options;
        let legendWidth = options.displayLegend ? (parent.Legend.getDim("width") + options.legendMargin[1] + options.legendMargin[3]) : 0;
        let legendHeight = options.displayLegend ? (parent.Legend.getDim("height") + options.legendMargin[0] + options.legendMargin[2]) : 0;

        let graphWidth = parent.graphDim.width - options.domainGutter - options.cellPadding;
        let graphHeight = parent.graphDim.height - options.domainGutter - options.cellPadding;
    ...
    setLegend() {
        ...
        let oldLegend = this.options.legend.slice(0);
    ...
    getSVG() {
        ...
        let filterStyles = function(attribute, property, value) {
        ...
        for (let element in styles) {
            ...
            if ("getComputedStyle" in window) {
                let cs = getComputedStyle(dom, null);
                if (cs.length !== 0) {
        ...
        let string = "<svg xmlns=\"http://www.w3.org/2000/svg\" " +
            "xmlns:xlink=\"http://www.w3.org/1999/xlink\"><style type=\"text/css\"><![CDATA[ ";

        for (let style in styles) {
            string += style + " {\n";
            for (let l in styles[style]) {
...
class Legend {
    ...
    computeDim() {
        ...
        let options = this.calendar.options; // Shorter accessor for variable name mangling when minifying
        this.dim = {
    ...
    redraw(width) {
        ...
        let legendItem;
        let options = calendar.options; // Shorter accessor for variable name mangling when minifying

        this.computeDim();

        let _legend = options.legend.slice(0);
        _legend.push(_legend[_legend.length - 1] + 1);

        let legendElement = calendar.root.select(".graph-legend");
    ...
    getDim(axis) {
        ...
        let isHorizontal = (this.calendar.options.legendOrientation === "horizontal");
    ...
    buildColors() {
        ...
        let legendColors = _legend.map(function(element) {
    ...
    getClass(n, withCssClass) {
        ...
        for (let i = 0, total = this.calendar.options.legend.length - 1; i <= total; i++) {
